...
#模型保存路径
model_save_path = "./save_model/part_B/CSR_train_rain2_BG_portion0.2_80.pkl"
...
def main():
    ...
    #这里是分配干净核脏的数据集的比例的 SHB是400张，80对应的就是20%，您看您怎么调整
    train_list = train_list_clean + train_list_dirty[0:80]
    ...
    #启动模型
    model = CSRNet()
    # model = torch.load("./save_model/part_B/train_rain_BG_portion0.2_80.pkl")
    #将模型传给gpu进行运算
    model = model.cuda()
    ...
    #############
        #所有的命名 Rain2_BG_portion0.2_80_MAE_eplips 需要您自己调整哦


    viz = Visdom()
    viz.line([[0.,0.]], [0.], win='Rain2_BG_portion0.2_80_MAE_eplips', opts=dict(title='Rain2_BG_portion0.2_80_MAE_eplips', legend=['MAE_clean', 'MAE_dirty']))
    viz.line([[0.,0.]], [0.], win='Rain2_BG_portion0.2_80_MSE_eplips', opts=dict(title='Rain2_BG_portion0.2_80_MSE_eplips', legend=['MSE_clean', 'MSE_dirty']))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        viz.line([[MAE_c_np, MAE_t_np]], [epoch], win='Rain2_BG_portion0.2_80_MAE_eplips', update='append')
        viz.line([[MSE_c, MSE_t]], [epoch], win='Rain2_BG_portion0.2_80_MSE_eplips', update='append')
        # 本地画图
        ...
        plt.xlabel("epoch")
        plt.ylabel("MSE")
        plt.savefig("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/MSE&MAE_Rain_BG_portion0.2_80_eplips.jpg")    

        # 文件记录折线图数据
        MAE_clean_file = open("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/MAE_clean_points.txt", 'a')
        MAE_triggered_file = open("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/MAE_triggered_points.txt", 'a')
        MSE_clean_file = open("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/MSE_clean_points.txt", 'a')
        MSE_triggered_file = open("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/MSE_triggered_points.txt", 'a')  
...
def validate(val_list, model, criterion, epoch):
    ...
    ax.set_title("gt")
    plt.colorbar(im2)
    fig.savefig("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/clean/{}_clean.jpg".format(epoch), dpi=500)
    ...
    ax.set_title("gt")
    plt.colorbar(im2)
    fig.savefig("./heatmaps/part_B/CSR_Rain2_BG_80_portion0.2/triggered/{}_triggered.jpg".format(epoch), dpi=500)
